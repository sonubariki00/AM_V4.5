...
def search_asset():
    ...
    log_manager.add_log('INFO', 'asset_management', f"Search request received for asset: {asset_name}")
    
    # Search for the asset in your database
    asset = get_asset_by_name(asset_name)
    if asset:
        log_manager.add_log('INFO', 'asset_management', f"Asset found: {asset_name}, ID: {asset['id']}")
        return jsonify({"asset_id": asset['id']}), 200
    else:
        log_manager.add_log('WARNING', 'asset_management', f"Asset not found: {asset_name}")
...
def add_employee():
    ...
    new_employee = {
        "id": employee_id,
        "name": employee_name,
        "password": "hashed_" + employee_password,  # Placeholder for the password hashing
        "role": "employee"
    }
...
def view_requests():
    log_manager.add_log('INFO', 'request_management', "View requests called by admin")
    ...
    log_manager.add_log('INFO', 'request_management', f"Returning {len(formatted_requests)} requests")
